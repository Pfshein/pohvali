...
**PH-704B · публичный запуск:** добавить выгрузку готового `*.dump.age` в
приватное внешнее object storage и выполнить restore drill при недоступном VPS.
Только после этого PH-704 считается выполненной полностью.
