...
## M9 — уютная главная «Комната»

Подробная архитектура: [`docs/superpowers/specs/2026-09-04-room-design.md`](superpowers/specs/2026-09-04-room-design.md).
Пошаговая реализация: [`docs/superpowers/plans/2026-09-04-room.md`](superpowers/plans/2026-09-04-room.md).

### PH-901 · P1 · Архитектурный каркас React + PixiJS

**Статус:** каркас подготовлен изолированно от текущего главного экрана; UX-интеграция остаётся в PH-902.

**Цель:** ввести PixiJS как узкий renderer комнаты, не смешивая canvas lifecycle
с похвалой, Telegram bootstrap, API и шифрованием.

**Границы:** frontend-only модель `RoomItem/RoomState`, нормализованные координаты,
слои, прямоугольные placement zones, versioned JSON, catalog/asset registry,
starter room, `RoomRuntime`, Pixi adapter и доступный React `RoomHome`. Бэкенд,
магазин, инвентарь, сложная анимация и включение экрана для пользователей не входят.

**Приёмка:** два экземпляра одного template независимы; resize не меняет state;
drag ограничен зоной; слой нельзя перепрыгнуть локальным z-index; malformed/unknown
JSON отвергается; mount/update/resize/destroy тестируются; RoomHome содержит
семантические действия «Похвалить себя», «Обустроить», календарь и профиль;
frontend quality gate зелёный.

### PH-902 · P1 · Интеграция RoomHome в текущий App

**Зависит от:** PH-901.

**Цель:** сделать комнату новой главной поверхностью, сохранив рабочий 15–20-секундный
путь похвалы и все существующие возможности.

**Границы:** декомпозиция текущего `App` на shell и панели; «Похвалить себя» открывает
существующий encrypted composer; Calendar/Profile открываются как отдельные sheets;
«Обустроить» переключает режим перемещения. `SessionRoot` и API-контракты не меняются.

**Приёмка:** после onboarding открывается почти fullscreen комната; текущий активный
маскот берётся из фактического `MascotCollection` вместе с `assetPath` (включая
добавленных admin-командой, без hardcode); создание/чтение/редактирование похвалы,
календарь, коллекция, reminders, recovery и privacy остаются доступны; ошибки Pixi
не блокируют DOM-кнопку похвалы; старые интеграционные тесты обновлены по новому flow.

### PH-903 · P1 · Локальное сохранение расстановки

**Зависит от:** PH-901, PH-902.

**Цель:** сохранять спокойную персональную расстановку на устройстве без новой
серверной сущности.

**Границы:** storage adapter по существующему паттерну Telegram CloudStorage с
localStorage fallback; ключ `pohvala.room.v1`; debounce записи после drop;
invalid/unknown version безопасно возвращает starter room. Личные тексты и ключ
шифрования в room JSON не входят.

**Приёмка:** расстановка восстанавливается после перезапуска; битое значение не
ломает приложение; смена размера viewport не переписывает JSON; смена активного
маскота сохраняет мебель и заменяет только экземпляр `active-mascot`.

### PH-904 · P2 · Каталог предметов и простой инвентарь

**Зависит от:** PH-903.

**Цель:** добавлять мебель декларативно через template/asset manifest и создавать
экземпляры без изменений RoomScene.

**Приёмка:** новый предмет требует записи manifest + template + renderer kind;
один template можно поставить несколько раз; удаление/возврат предмета обратимы;
неизвестный asset показывает placeholder; лицензия и источник каждого production
asset зафиксированы.

### PH-905 · P2 · Реакция комнаты на похвалу

**Зависит от:** PH-902.

**Цель:** после успешного сохранения дать короткую тёплую реакцию маскота, не
превращая её в награду за непрерывность.

**Границы:** одноразовая runtime-команда `praiseSaved`; idle/reaction systems,
reduced-motion fallback и краткоживущий effects layer. Событие не сериализуется.

**Приёмка:** реакция запускается только после успешного API-ответа; повторный render
не повторяет её; reduced motion показывает статичное изменение; частицы очищаются
и не накапливают display objects/listeners.

### PH-906 · P2 · Серверная синхронизация комнаты

**Зависит от:** подтверждённой потребности после PH-903/PH-904.

**Цель:** переносить расстановку между устройствами только если локального хранения
недостаточно по результатам теста.

**Приёмка:** отдельный versioned payload без текста похвал и Telegram PII; bounded
размер/число экземпляров; ownership на каждом запросе; конфликтная стратегия и
миграции схемы задокументированы до добавления таблицы. До подтверждения задачи
backend не меняется.
...
- **R4 / Cozy room:** PH-901–903; PH-904–906 после продуктовой проверки.
