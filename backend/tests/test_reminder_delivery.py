...
from app.modules.reminders.delivery import SendOutcome, send_with_backoff
from app.modules.reminders.sender import ReminderThrottled, ReminderUnavailable
...
def test_backoff_gives_up_after_the_attempt_budget() -> None:
    ...
    assert outcome is SendOutcome.THROTTLED
    assert sender.calls == 3
    assert len(slept) == 2  # no sleep after the final failed attempt


def test_unavailable_chat_is_terminal_without_retries() -> None:
    ...
    assert outcome is SendOutcome.UNAVAILABLE
