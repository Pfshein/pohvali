...
и обновляет его после покупки или выбора.
...
кроме тестов). Отправка и затухание — PH-503, туда же подставляется обработчик кандидатов.
Job — in-process, поэтому backend должен быть в одном экземпляре (PH-705).
